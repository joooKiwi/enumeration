...
describe("CollectionHolderTest", () => {
    ...
    describe("filter", () => {
        ...
        test("[a,b,1,2].filter(number) == [1,2]", () => expect(AB12.filter((it): it is number => typeof it == 'number',).toArray(),).toStrictEqual([1, 2,],),)
        test("[a,b,1,2].filterNot(number) == [a,b]", () => expect(AB12.filterNot((it): it is number => typeof it == 'number',).toArray(),).toStrictEqual(['a', 'b',],),)
